package com.taosha.summaryframework.customData.utils;
...
public class JdbcUtils {
        ...
        if (entityClass != null) {
            ...
            for(int var9 = 0; var9 < var8; ++var9) {
                ...
                if (entityToDbMap.get(entityField.getName()) != null) {
                    ...
                    log.warn("entity中的属性'{}'找不到对应的数据库'{}'字段.", entityField.getName(), tableName);
                }
            }
    ...
    public static JdbcUtils.JdbcResult getInsert(String tableName, Object entity, Map<String, String> entityToDbMap) {
        ...
        return getInsert(sql, entity, entityToDbMap);
    }

    public static JdbcUtils.JdbcResult getInsertIgnore(String tableName, Object entity, Map<String, String> entityToDbMap) {
        StringBuilder sql = (new StringBuilder("INSERT IGNORE INTO ")).append(tableName);
        return getInsert(sql, entity, entityToDbMap);
    }

    public static JdbcUtils.JdbcResult getInsert(StringBuilder sql, Object entity, Map<String, String> entityToDbMap) {
        Map<String, Object> dbMap = transferEntityToDbMap(entity, entityToDbMap);
        if (dbMap != null && dbMap.size() > 0) {
            Object[] params = new Object[dbMap.size()];
            int index = 0;
            sql.append("(");

            Iterator var6;
            Entry entry;
            for(var6 = dbMap.entrySet().iterator(); var6.hasNext(); ++index) {
    ...
    public static String getNamedInsert(String tableName, Class<?> cls, Map<String, String> entityToDbMap) {
        ...
        for(int var8 = 0; var8 < var7; ++var8) {
            ...
            if (StringUtils.isBlank(colName)) {
                log.warn("类[{}]field[{}]没有对应的数据库字段", cls.getSimpleName(), fieldName);
            } else {
                props.add(fieldName);
                if (index != 0) {
                    sql.append(",");
        ...
        try {
            ...
            for(int var6 = 0; var6 < var5; ++var6) {
                ...
                if (f.get(obj) != null) {
                    String dbColumn = (String)entityToDbMap.get(f.getName());
                    if (dbColumn == null) {
                        log.warn("类'{}'中属性'{}'没有对应的数据库字段", obj.getClass().getSimpleName(), f.getName());
                    } else {
